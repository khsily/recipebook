...
UNFREEZE_TRAIN = True
UNFREEZE_TRAIN_LR = 0.000001
UNFREEZE_TRAIN_BATCH_SIZE = 5
...
def _main():
    ...
    if FROZEN_TRAIN:
        model.compile(optimizer=Adam(lr=FROZEN_TRAIN_LR), loss={
        ...
        model.save_weights(os.path.join(log_dir, FROZEN_TRAIN_OUTPUT_WEIGHTS))
    ...
    # Save final weights.
    model.save_weights(os.path.join(log_dir, FINAL_OUTPUT_WEIGHTS))
...
def data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes):
    ...
    while True:
        ...
        for b in range(batch_size):
            ...
            image, box = get_random_data(annotation_lines[i], input_shape, random=IMAGE_AUGMENTATION)
