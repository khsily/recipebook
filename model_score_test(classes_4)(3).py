...
def rpn_layer(base_layers, num_anchors):
    ...
    Returns:
        ...
        base_layers: vgg in here
    """
    x = tf.keras.layers.Conv2D(512, (3, 3), padding='same', activation='relu', kernel_initializer='normal', name='rpn_conv1')(base_layers)
...
def classifier_layer(base_layers, input_rois, num_rois, nb_classes=5):
    ...
    out_class = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(nb_classes, activation='softmax', kernel_initializer='zero'),
                                name='dense_class_{}'.format(nb_classes))(out)
    # note: no regression target for bg class
    out_regr = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(4 * (nb_classes - 1), activation='linear', kernel_initializer='zero'),
...
def rpn_loss_regr(num_anchors):
    ...
    def rpn_loss_regr_fixed_num(y_true, y_pred):
        ...
        return lambda_rpn_regr * tf.keras.backend.sum(
            y_true[:, :, :, :4 * num_anchors] * (x_bool * (0.5 * x * x) + (1 - x_bool) * (x_abs - 0.5))) / tf.keras.backend.sum(epsilon + y_true[:, :, :, :4 * num_anchors])
...
def rpn_loss_cls(num_anchors):
    ...
    def rpn_loss_cls_fixed_num(y_true, y_pred):
            ...
            return lambda_rpn_class * tf.keras.backend.sum(y_true[:, :, :, :num_anchors] * tf.keras.backend.binary_crossentropy(y_pred[:, :, :, :], y_true[:, :, :, num_anchors:])) / tf.keras.backend.sum(epsilon + y_true[:, :, :, :num_anchors])
...
def class_loss_regr(num_classes):
    ...
    def class_loss_regr_fixed_num(y_true, y_pred):
        ...
        return lambda_cls_regr * tf.keras.backend.sum(y_true[:, :, :4*num_classes] * (x_bool * (0.5 * x * x) + (1 - x_bool) * (x_abs - 0.5))) / tf.keras.backend.sum(epsilon + y_true[:, :, :4*num_classes])
    return class_loss_regr_fixed_num


def class_loss_cls(y_true, y_pred):
    return lambda_cls_class * tf.keras.backend.mean(tf.keras.losses.categorical_crossentropy(y_true[0, :, :], y_pred[0, :, :]))
...
base_path = 'C:\\Users\\Administrator\\PycharmProjects\\test4'
# train_path =  'C:\\Users\\Administrator\\PycharmProjects\\test4\\test_annotation.txt'
test_base_path = 'C:\\Users\\Administrator\\PycharmProjects\\test4\\test' # Directory to save the test images
...
roi_input = tf.keras.Input(shape=(C.num_rois, 4))
feature_map_input = tf.keras.Input(shape=input_shape_features)
...
def test_show():
    ...
    for idx, img_name in enumerate(imgs_path):
        ...
        # R.shape = (300, 4)
        R = rpn_to_roi(Y1, Y2, C, tf.keras.backend.image_data_format(), overlap_thresh=0.7)
